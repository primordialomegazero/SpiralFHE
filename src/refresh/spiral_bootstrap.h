...
// Supported a-values for Cassini invariant (skip a=2).
// a=2 has a zero crossing at s=0.374653... where C=0.
// All other a in {1,3,4,5,6,7} have min(C) > 0.1 for all s in [0,1).
// Verified: 1M samples per a-value, 0 below tau.
...
struct GFNLayer {
    ...
    int a_value;
    bool valid;
    GFNLayer() : y1(0.0), y2(0.0), seed(0.0), cassini(0.0), a_value(1), valid(false) {}
};
...
private:
    ...
    double cached_seed;
    int rotation_count;
    static_assert(N <= MAX_LAYERS, "N must be <= 6. Supported a-values: {1,3,4,5,6,7}");
...
public:
    ...
    void initialize(double seed) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            layers[i].cassini = fabs((layers[i].y1 + a * SpiralConstants::PHI) * 
                                     (layers[i].y2 + a * SpiralConstants::PSI) + 1.0);
    ...
    double compute_cassini_for_layer(int i) const {
        ...
        int a = layers[i].a_value;
        double pc = layers[i].y1 + a * SpiralConstants::PHI;
        double qc = layers[i].y2 + a * SpiralConstants::PSI;
...
private:
    ...
    double compute_phi_from_operational(const OperationalState& op) {
        double health_fraction = (op.total_layers > 0) ? (double)op.healthy_layers / op.total_layers : 0.0;
        double connectivity = 0.0;
        if (level1_history.size() >= 2) {
            for (size_t i = 1; i < level1_history.size(); i++)
                connectivity += fabs(level1_history[i].integrated_phi - level1_history[i-1].integrated_phi);
            connectivity /= (level1_history.size() - 1);
        }
        double cassini_signal = op.cassini_min;
        double cassini_penalty = (cassini_signal < SpiralConstants::CASSINI_THRESHOLD * 3) ? (1.0 - cassini_signal) : 0.0;
        double level_fraction = (op.ckks_max_level > 0) ? (double)op.ckks_level / op.ckks_max_level : 0.0;
    ...
    MetaControllerState compute_meta_state(const OperationalState& op) {
        ...
        if (level1_history.size() < 10) {
            ...
            meta.learning_rate = INV_PHI; meta.adjusted_threshold = SpiralConstants::CASSINI_THRESHOLD * 3;
            meta.learned_min_level = 4; meta.parameters_stable = false;
            return meta;
        }
        ...
        meta.adjusted_threshold = SpiralConstants::CASSINI_THRESHOLD * (1.0 + meta.learning_rate);
        ...
        if (level_samples.size() >= 5) {
            ...
            meta.learned_min_level = min_seen + (uint32_t)(range * meta.learning_rate * 0.5);
            if (meta.learned_min_level < 2) meta.learned_min_level = 2;
            if (meta.learned_min_level > 8) meta.learned_min_level = 8;
        } else { meta.learned_min_level = 4; }
        meta.parameters_stable = (meta.threshold_stability > 0.9) && (meta.phi_convergence_rate > 0.5);
        return meta;
    }

    double compute_variance(typename std::deque<ControllerState>::const_iterator b,
                            typename std::deque<ControllerState>::const_iterator e) {
        if (b == e) return 0.0;
...
public:
    RecursiveFractalController() {
        ...
        current_meta.learning_rate = INV_PHI; current_meta.adjusted_threshold = SpiralConstants::CASSINI_THRESHOLD * 3;
        current_meta.learned_min_level = 4; current_meta.parameters_stable = false;
    }
    ...
    ControllerState update(const OperationalState& op_state) {
        ...
        if (level1_history.size() >= 5) {
            ...
            ctrl.stability = 0.0;
            for (double v : recent) ctrl.stability += (v - mean) * (v - mean);
            ctrl.stability = 1.0 / (1.0 + ctrl.stability / recent.size());
        } else { ctrl.stability = 0.0; }
        ctrl.should_bootstrap = (op_state.ckks_level <= current_meta.learned_min_level) ||
                                (op_state.cassini_min < current_meta.adjusted_threshold) ||
                                (op_state.healthy_layers < op_state.total_layers) ||
                                (op_state.cassini_min < SpiralConstants::CASSINI_THRESHOLD * 2);
        ...
        if (level2_convergence_series.size() > HistorySize) level2_convergence_series.pop_front();
        detect_fractal();
        return ctrl;
    }
...
private:
    ...
    int bootstrap_count;
    bool initialized;
    static_assert(GFNLayers <= MAX_LAYERS, "GFNLayers must be <= 6. Supported a-values: {1,3,4,5,6,7}");
